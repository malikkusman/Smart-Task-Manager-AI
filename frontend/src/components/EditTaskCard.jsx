...
const EditTaskCard = ({ _id, setViewEditModel, setRefetching }) => {
  // Add 'setRefetching' to the props validation
  EditTaskCard.propTypes = {
    _id: PropTypes.string.isRequired,
    setViewEditModel: PropTypes.func.isRequired,
  ...
  const fetchTask = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get(`/tasks/${_id}`);
      setTask(data);
      setNotes(data.notes);
  ...
  const handleEditTask = async () => {
    try {
      setIsUpdating(true);
      await axios.put(`https://task-ai.onrender.com/tasks/${_id}`, {
        ...task,
        notes,
        priority,
      });
  ...
  const renderContent = () => {
    ...
    if (!task) {
      return <p className="text-medium text-text text-center">Task Not Found</p>;
    }
    ...
    return (
      <>
        ...
        <h1 className="my-4 text-text font-poppins capitalize text-large font-[400]">
          {task.title}
        </h1>
  ...
  return (
    <div
      ...
      onClick={() => {
        if (!isDone && !isUpdating) setViewEditModel(false);
      }}
    >
      <div
